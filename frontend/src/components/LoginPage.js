...
export default function LoginPage() {
  ...
  const googleSignIn = () => {
    axios
      .get(
        `http://localhost:8000/auth/o/google-oauth2/?redirect_uri=http://localhost:8000/explore`
      )
      .then(
        (response) => {
  ...
  const fbSignIn = () => {
    axios
      .get(
        `http://localhost:8000/auth/o/facebook/?redirect_uri=http://localhost:8000/explore`
      )
      .then(
        (response) => {
          window.location.replace(response.data.authorization_url);
